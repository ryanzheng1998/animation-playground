...
function App() {
  ...
  const backgroundStyle = useSpring({
    background: hidden ? 'pink' : 'white',
    scale: hidden ? 0.1 : 1,
    // don't know how to do this in css
    // borderRadius: `/ calc(#{$r}/var(--fy))`,
    delay: hidden ? 600 : 0,
    config: config.stiff,
  })

  const [listStyle] = useTrail(
  ...
  return (
    <div className="bg-blue-400 p-5 h-screen">
      <a.div
        ...
        className="grid grid-cols-4 gap-4 h-full rounded p-8 cursor-pointer"
        style={backgroundStyle}
      >
        {list.map((x, i) => {
          const style = listStyle[Math.floor(i / 4)]

          return (
            <a.div
              key={x.name}
              className="rounded"
              style={{ ...style, background: x.css }}
            />
          )
        })}
      </a.div>
